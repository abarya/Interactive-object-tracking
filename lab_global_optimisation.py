'''This is the implementation of a tracking by detection framework in which a likelihood map is 
   ...
   After this, we compute the optimal path using Dynamic programming. Likelihood maps are based on the paper by Possegger but using 
   CIE lab color space. In this, we can manually annotate the object on any frame if we feel that none of the top 6 windows are on the object.
   After reannotation, our object model is updated. '''
...
if __name__ == "__main__":
	...
	if (len(argument)<2) :
		...
		if var==1 :
			...
			i=0
			while(1) :
				cap.set(cv2.CAP_PROP_POS_FRAMES,int(frame_count-i-1))
				ret,img=cap.read()
				if ret==1 :
					break
				i=i+1
			cap.set(cv2.CAP_PROP_POS_FRAMES,0)
			num_images=int(frame_count-i)
	...
	for i in range(num_anno) :
		if i==0 :
			...
			list_refpt=[]
			cap.set(cv2.CAP_PROP_POS_FRAMES,num_images-200)
			ret,image=cap.read()
			image=cv2.resize(image,(500,500))
			image=cv2.cvtColor(image,cv2.COLOR_BGR2Lab)
			images.append(image)
	...
	for i in range(num_anno) :
		...
		dist_img_list = get_distractor(list_score,score_obj,object_window_list[i],image)
	...
	hstack1=np.hstack((color_map1,color_surr))
	hstack2=np.hstack((color_dist,color_map2))
	vstack=np.vstack((hstack1,hstack2))
	vstack=cv2.resize(vstack,(1000,600))
	cv2.imshow("1. combined image, 2. obj-surr image, 3. obj-dist image, 4. combined image(last frame) ",vstack)
	cv2.waitKey(0)
	cv2.destroyAllWindows()
	...
	while(i<num_images) :
		...
		if i==1 :
			...
			det_term=-math.log(DP_score[0,num_images-1]/(h*w))
		...
		if k== 27 :
			flag=1
			break
		# if anyone wants to reannotate, press "r" and then this if block will be exexuted
